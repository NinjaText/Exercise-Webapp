...
function Bar({ label, consumed, target, unit, icon: Icon, soft, text, dot }: MacroRow) {
  ...
  return (
    <div className="flex items-center gap-3">
      <div className={cn("flex h-8 w-8 shrink-0 items-center justify-center rounded-lg", soft)}>
      ...
      <div className="min-w-0 flex-1 space-y-1">
        ...
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
          <div
            className={cn("h-full rounded-full transition-all duration-300", exceeded ? "bg-danger" : dot)}
            style={{ width: target ? `${width}%` : "0%" }}
          />
        </div>
      </div>
    </div>
...
export function MacroProgressBars({ calories, proteinG, carbsG, fatG }: MacroProgressBarsProps) {
  return (
    <div className="grid grid-cols-1 gap-x-8 gap-y-4 sm:grid-cols-2">
      ...
      <Bar
        label="Protein"
        consumed={proteinG.consumed}
        target={proteinG.target}
        unit="g"
        icon={Beef}
        soft="bg-info-soft"
      ...
      <Bar
        label="Carbs"
        consumed={carbsG.consumed}
        target={carbsG.target}
        unit="g"
        icon={Wheat}
